...
let spaceshipData = []
let spaceshipDataIndex = []
let currentSpaceshipData
let currentSpaceshipDataIndex
...
function getKeyframeObject(index, timestamp, type) {
	// console.log(type)
	return spaceshipData[index].keyframeData[spaceshipDataIndex[index][timestamp]]
}
...
function loop(timestamp) {
	...
	while (currentSpaceshipDataIndex.length < currentKeyframeTimestampIndex) {
		// await timeoutPromise(10)
		// console.log('in the while loop . . .')
		csdiLength = currentSpaceshipDataIndex.length
		if (currentSpaceshipDataIndex[csdiLength] === undefined) {
			currentSpaceshipDataIndex[csdiLength] = []
		}
		currentSpaceshipDataIndex[csdiLength].push(currentSpaceshipData.keyframeData.length)
	}
	...
	p.speed.y += p.attributes.gravity
	...
	if (currentSpaceshipData !== undefined && (currentSpaceshipData.endLocation.x <= (p.x + 12) && (currentSpaceshipData.endLocation.x + 22) >= p.x) && (currentSpaceshipData.endLocation.y <= (p.y + 12) && (currentSpaceshipData.endLocation.y + 22) >= p.y)) {
		...
		spaceshipDataIndex.push(currentSpaceshipDataIndex)
